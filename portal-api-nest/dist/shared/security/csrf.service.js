"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.CsrfService = void 0;
const common_1 = require("@nestjs/common");
const crypto_1 = require("crypto");
const session_token_service_1 = require("./session-token.service");
const cookies_service_1 = require("./cookies.service");
let CsrfService = class CsrfService {
    tokenService;
    cookiesService;
    constructor(tokenService, cookiesService) {
        this.tokenService = tokenService;
        this.cookiesService = cookiesService;
    }
    generarCsrfToken() {
        return `csrf_${(0, crypto_1.randomUUID)()}`;
    }
    hashCsrfToken(token) {
        return this.tokenService.hashToken(token);
    }
    validarCsrfRequest(request) {
        const headerToken = request.headers['x-csrf-token']?.trim();
        if (!headerToken)
            return null;
        const cookieToken = this.cookiesService.readCookie(request, this.cookiesService.csrfCookiePolicy().name);
        if (!cookieToken)
            return null;
        return headerToken === cookieToken ? headerToken : null;
    }
};
exports.CsrfService = CsrfService;
exports.CsrfService = CsrfService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [session_token_service_1.SessionTokenService,
        cookies_service_1.CookiesService])
], CsrfService);
